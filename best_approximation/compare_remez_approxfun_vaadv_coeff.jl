...
function main(args::Vector{String} = ARGS)
    ...
    lo = parse(Float64, get(kv, "lo", string(-Float64(pi))))
    hi = parse(Float64, get(kv, "hi", string(Float64(pi))))
    h = parse(Float64, get(kv, "h", "0.05"))
    ...
    setprecision(precision_bits) do
        ...
        f_ref_big = Fun(x -> hB * (one(BigFloat) + (BigFloat(2) / BigFloat(3)) * sin(x)), Sbig)
        ...
        for n in degrees
            c64, _, _, _ = remez_chebyshev(f, lo, hi, n; T = Float64, maxiter = maxiter)
            ...
            cbig, _, _, _ = remez_chebyshev(
                x -> hB * (one(BigFloat) + (BigFloat(2) / BigFloat(3)) * sin(x)),
                loB,
                hiB,
                n;
                T = BigFloat,
    ...
    p_l2 = plot(
        ...
        title = L"f(x)=\frac{1}{20}\left(1+\frac{2}{3}\sin x\right),\ x\in[-\pi,\pi]",
    )
    ...
    out_l2 = joinpath(@__DIR__, "vaadv_coeff_error_L2.png")
    savefig(p_l2, out_l2)
    out_l2_pdf = joinpath(@__DIR__, "vaadv_coeff_error_L2.pdf")
    out_l2_svg = joinpath(@__DIR__, "vaadv_coeff_error_L2.svg")
    ...
    out_linf = joinpath(@__DIR__, "vaadv_coeff_error_Linf.png")
    savefig(p_linf, out_linf)
    out_linf_pdf = joinpath(@__DIR__, "vaadv_coeff_error_Linf.pdf")
    out_linf_svg = joinpath(@__DIR__, "vaadv_coeff_error_Linf.svg")
